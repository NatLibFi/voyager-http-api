...
(function() {
  ...
  var MARC_EXTENDED_NAMESPACE = 'http://linneatest.csc.fi/marcextended',
  ...
  module.exports = function(options, dbConnection, createVoyagerBatchcatCustom) {
          ...
          if (Array.isArray(record)) {
            ...
            try {
              if (request.method === 'POST') {
                ...
                return utils.getRecordUpdateDate(dbConnection, request.params.id, options.db).then(
                  function (updateDate) {
    ...
    router.route('/:id/history').get(routeFoundCallback, function(request, response, next) {
      return utils.getRecordHistory(dbConnection, request.params.id, options.db).then(
        function(data) {
  ...
  .get(routeFoundCallback, function(request, response, next) {
    return utils.getRecord(dbConnection, request.params.id, options.db).then(
      function(record) {
        try {
          response.status(200).type('application/xml').send(utils.serializeRecord(record));
          next();
        } catch (error) {
          next(error);
        }
