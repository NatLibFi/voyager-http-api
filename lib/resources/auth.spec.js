...
const invalidFakeRecordData = fs.readFileSync(path.resolve(__dirname, '../../test-utils/fake-data/invalid-record-xml-slim.xml'), 'utf8');
const invalidFakeRecord = utils.parseRecord(invalidFakeRecordData);
...
describe('Authority Routehandler', () => {
  ...
  describe('put authority record', function() {
    ...
    it('should respond with 400 BAD_REQUEST if there are missing query parameters', () => {
    ...
    it('should respond with 400 BAD_REQUEST if record is in invalid format', () => {
      const fakeCatLocationId = '95';
    ...
    it('should respond with 400 BAD_REQUEST with message if record contains invalid fields', () => {
      const fakeCatLocationId = '95';
      
      // Batchcat will return success without actually taking any action if authority record contains non-numeric fields
      authorityServiceMock.update.resolves();
      return request(app)
        .put('/auth/123')
        .query({ catLocation: fakeCatLocationId })
        .auth(fakeCreds.username, fakeCreds.password)
        .send(invalidFakeRecordData)
        .type('application/xml')
        .expect(HttpStatus.BAD_REQUEST)
        .then(response => {
          expect(response.body.error.message).to.contain('The following fields are not supported');
          expect(response.body.error.message).to.contain('CAT');
          expect(response.body.error.message).to.contain('LOW');
        });
    });

    it('should respond with 404 NOT_FOUND if record does not exist', () => {
      const fakeCatLocationId = '95';
