...
(function() {
  ...
  module.exports = {
    getRecord: function(connection, id, options) {
      var SQL_QUERY = 'SELECT marc_record FROM '+options.name+'.'+options.type+'blob_vw WHERE '+options.type+'_id = :id';

      return connection.execute(SQL_QUERY, [recordId], {resultSet: true}).then(
        function(result) {
          return result.resultSet.getRow().then(function(data) {
            return result.resultSet.close().then(function() {

              console.log(data);
              return utils.parseRecord(data);
    ...
    getRecordHistory: function(connection, id, options) {
      ...
      var SQL_QUERY = 'SELECT action_date, location_id, action_type_id, e, s FROM '+options.name+'.'+options.type+'_history WHERE '+options.type+'_id = :id ORDER BY action_date desc';
