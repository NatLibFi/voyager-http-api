...
(function() {
  ...
  module.exports = {
    getRecord: function(connection, id, options) {
      var SQL_QUERY = 'select utl_raw.CAST_TO_RAW(RECORD_SEGMENT) as SEG from '+options.name+'.'+options.type.toUpperCase()+'_DATA where '+options.type.toUpperCase()+'_id = :id order by SEQNUM';
